...
# ... Lots of stuff we forgot to record here

# x) transformed ice covvariates to principal components
...
parameters <- c("mean.phi0", "mean.phiA", "mean.log.F", "betaN", "betaPhi", "sigma.o", 
                "sigma.BP", "sigma.phi0", "sigma.phiA", "phi.corr",
                "sigma.p", "beta.nest","Nb", "phiA", "phi0", "F",  "alpha", 
                "pd", "o", "sign2","pd2", "pext",  "BP", "p")

# MCMC settings
ni <- 60000; nt <- 1; nb <- 30000; nc <- 3 # up'd iterations from 20K
...
sigma.phi0 ~ dgamma(2, 4)
tau.phiA <- pow(sigma.phiA, -2)
sigma.phiA ~ dgamma(2, 4)
phi.corr ~ dbeta(2, 1)
phi.sigma[1,1] <- sigma.phi0^2
phi.sigma[2,2] <- sigma.phiA^2
phi.sigma[1,2] <- sigma.phi0*sigma.phiA*phi.corr
phi.sigma[2,1] <- sigma.phi0*sigma.phiA*phi.corr
phi.mu[1] <- 0
phi.mu[2] <- 0
...
for (t in 1:(BEFORE+n.occasions-1+AFTER+K)){ 
    ...
    N[2,t+1] <- (phi1[t] + (1-phi1[t])*omegaJ)*(1-alpha)*N[1,t]
    N[3,t+1] <- (phi1[t] + (1-phi1[t])*omegaJ)*alpha*N[1,t]
...
for (t in 1:(n.occasions+BEFORE+AFTER)){
  dev[t] ~ dbeta(2.16, 11.56) # test effect of d contrasted to those from EE, see ObsPriors.R
  dev2[t] ~ dbeta(2.16, 11.56) T(0, 1/vcf[t])
  sign[t] ~ dbin(0.5, 1)
  pd[t] <- ifelse(sign[t] == 1, dev[t], -dev2[t])
  pext[t] ~ dbeta(1, 1)
  sign2[t] ~ dbin(pext[t], 1)
  pd2[t] ~ dgamma(4, 2)
  d[t] <- ifelse(sign2[t] == 1, pd[t], pd2[t])
...
for (t in 1:(n.occasions+BEFORE+AFTER+K-1)){ # extended loop here # should go to end - 1
  logit.BP[t] <- mean.logit.BP + eps.BP[t] #betaBP[1] + betaBP[2]*ext.ice[t] + betaBP[3]*ext.ice[t]*ext.ice[t] 
...
for (t in 1:(n.occasions - 1 + BEFORE + AFTER + K)){
    ...
    logit.phi0[t] <- betaPhi[1] + betaPhi[3]*pca.ice[t+1]
                     + betaPhi[4]*pca.ice[t+1]*pca.ice[t+1] + eps.phi[1,t] 
    ...
    logit.phiA[t] <- logit(ilogit(betaPhi[1] + betaPhi[2])*(1-lead[t]*(1-kappa))) 
                     ...
                     - betaN[2]*(N[3,t] + N[4,t])/1000 + eps.phi[2,t] 
...
saveRDS(out, file = "MS_Scenarios/YKD.constant.pca.gamma.noBPcov.rds")
...
set.seed(85)
hold <- sample(c(1:30000), 2000, replace = FALSE)

# subsampling the posterior draws of Nb
dat <- post$Nb[hold,]
# getting the median Nb for each year
Nbmed <- data.frame(apply(dat, 2, median))
names(Nbmed)[names(Nbmed) == 'apply.dat..2..median.'] <- 'Med'
Nbmed$Year <- c(1988:2100)
# grabbing 50 trajectories from the Nb subsample as examples
set.seed(45)
hold <- sample(1:nrow(dat), 50, replace = FALSE)
traj <- data.frame(t(dat[hold,]))

traj$Year <- c(1988:2100)
traj <- traj %>%
  gather(Sim, Nb, -Year)
traj$Sim <- as.factor(traj$Sim)

counts.YKD <- read.csv("input_data/YKD_SPEI.csv", header = T)

ggplot(traj, aes(x = Year, y = Nb/1000, col = Sim)) +
  geom_line() + 
  geom_line(data = Nbmed, aes(x = Year, y = Med/1000), size = 1.1, col = "black") + 
  geom_point(data = counts.YKD, aes(x = Year, y = Nibb/1000), size = 2, shape = 1, col = "black") +
  ...
  scale_color_manual(values=rep("grey80", length(traj$Sim))) + 
  ylim(0, 50) +
  theme_classic() + 
