# Based on IMP_YKD_26.R but for the ACP survey. Main differences:
#   (1) uses ACP data, see below; 
#   (2) fewer scenarios: current conditions, RCP4.5, RCP8.5 (no lead);
#   (3) because ACP data starts in 2007, need to priors for years 1988 to 2006 
#       to sync up with YKD and mark-recapture data,
#   (4) revised betaN prior based on DDpriorsEO.R,
#   (5) uses different initial population size based on Amundsonet al. 
#
#   All else is the same as YKD model. Importantly, mark-recapture data and nest 
#     data from YKD are used. This mean ACP get survival and fecundity shared with YKD. 
...
#               (6) used informed prior for alpha and p from CJS model because simulation some it is identifiable
...
################################################################################
#Define JAGS model
cat(file = "ACP_IPM.jags", "
model {

## Priors
# state-space
for(i in 1:19){ 
  sigma.obs[i] ~ dunif(800, 1400) }
# state-space
sigma.obs[33] ~ dunif(800, 1400) # 2020, covid years missing
sigma.obs[34] ~ dunif(800, 1400) # 2021 
...
sigma.BP ~ dunif(0,1)
for (i in 1:2){
betaN[i] ~ dgamma(3.02, 158.30) # from EE, see DDpriorsEO.R 
} #i
...
## State-space model for count data, look at Amundson paper ~ 4 (3 - 6)K
## assume 0.75 detection, translates to ~5 (4-8)K -- for 2*N[3:4,1]
# Model for the initial population size
N[1,1] ~ dunif(250, 500) # n1, 1YO 
N[2,1] ~ dunif(250, 500) # n2nB, 2YO non-breeding
N[3,1] ~ dunif(500, 1000) # n2B, 2YO breeding
N[4,1] <- N[5,1] + N[6,1] # n3+, 3+YO breeding
N[5,1] ~ dunif(500, 1000) # n3+ from n2nB
N[6,1] ~ dunif(1000, 2000) # n3+ from n2B and n3+
...
for (t in 1:(n.occasions+BEFORE+AFTER)){
  x[t] ~ dbeta(2.8, 2.8) # from EE, see ObsPriors.R
  d[t] <- log( 1 + (x[t] * (1 - 0.5) + 0.5 - 0.75)/0.75 ) #standarize to (0.5, 1) and make multiplicative on log scale 
...
K <- 74
# Count Data, 2007 - 2026
# ACP does not have a vcf correction factor but does have double observer 
#   detection estimates from Wilson et al and Bradley (reported in Sup. Mat. of Osnas 2026).
#   Wilson gives an approximate detection for front seat of 0.75, se ~= 0.06
#   Bradley report a detection estimate (average over distance bins) of 0.74, se ~= 0.04
#   We will use 0.75, se = 0.05
eiders <- AKaerial::ACPHistoric$combined |> filter(Species == "SPEI") |>
  mutate(Nibb = ibb/0.75, seNibb = sqrt( (ibb/0.75)^2 * ( (ibb.se/ibb)^2 + (0.05/0.75)^2 ) ),
         lower = Nibb - 2 * seNibb, upper = Nibb + 2 * seNibb)
write.csv(eiders, file = "input_data/ACP_counts_2026.csv")
ggplot(data = eiders, aes(x = Year, y = Nibb)) + 
  geom_pointrange(aes(ymin = lower, ymax = upper), linewidth = 1.1, color = "black") +
  geom_pointrange(aes(x = Year + 0.3, y = ibb, ymin = ibb - 2*ibb.se, 
                      ymax = ibb + 2 * ibb.se), linewidth = 1.1, color = "darkgray") +
  labs(title = "", x = "Year", y = "Indicated Breeding Birds") + 
  scale_x_continuous(breaks = seq(1988, 2026, by = 2), 
                     limits = c(1988 - 0.5, 2026 + 0.5))

counts <- eiders  |> 
  select(Year, Nibb, seNibb) 

df1 <- AKaerial::ACPHistoric$output.table |> filter(Species == "SPEI") |> 
  select(Year, obs1 = Observer) |> group_by(Year) |> slice(1) |> ungroup() |>
  complete(Year = min(Year):max(Year))
df2 <- AKaerial::ACPHistoric$output.table |> filter(Species == "SPEI") |> 
  select(Year, obs2 = Observer) |> group_by(Year) |> slice(2) |> ungroup() |>
  complete(Year = min(Year):max(Year))
df <- cbind(df1, df2) |> unite(col = "Team", obs1, obs2, sep = "_") |>
  mutate(Team = ifelse(Team == "NA_NA", "DES_HMW", Team)) #assume missing years 
                                                          #would have been flown by previous team

observer <- as.numeric(factor(c(rep("RMD_WWL", 19), df$Team))) #assume 1988:2006 team is RMD_WWL
####################################
...
inits <- function(){list(
  ...
  sigma.p = 0.5)}
# make gap years
gap_calendar_years <- c(25:27, 29) + 4 #missing re-sight in calendar years 2016, 2017, 2018, and 2020
# resight.index is indexed by m-array recapture occasion = calendar year - 1
gap_occasions <- gap_calendar_years - 1
resight.index <- rep(1, ncol(ch) - 1 + 4)
...
jags.data8.5 <- list(obs.ice = ice.data$ice_8.5, nb.size = theta8.5, muBP = muBP, SigmaBP=SigmaBP,
                     ...
                     ones = diag(5), 
                     count = c(rep(NA, 19), counts$Nibb), 
                     obs = observer, 
                     sigma.obs = c(rep(NA, 19), counts$seNibb),
                     nest.obs.a = c(fecund.param$ns.obs.alpha, ns.a),
                     nest.obs.b = c(fecund.param$ns.obs.beta, ns.b),
                     K = K, BEFORE = 4, AFTER = 1)
saveRDS(jags.data8.5, file = "MS_Scenarios/jags.data8.5.rds")

# parameters monitored
parameters <- c("Nb", "phiA", "phi0", "F", "mean.phi0", "mean.phiA", "mean.alpha", 
                "mean.log.F", "betaN", "beta", "sigma.o", "o", "d", "mean.p", "p",
...
# ice 8.5
start <- Sys.time()
out <- jags(jags.data8.5, inits, parameters, "ACP_IPM.jags", 
                      n.chains = nc, n.burnin=nb, n.iter = ni,  
                      parallel = TRUE, n.adapt = 1000)
Sys.time() - start
saveRDS(out, file = "MS_Scenarios/ACP.8.5.rds")
################################################################################
################################################################################
#ice 4.5
...
start <- Sys.time()
out <- jags(jags.data4.5, inits, parameters, "ACP_IPM.jags", 
                      n.chains = nc, n.burnin=nb, n.iter = ni,  
                      parallel = TRUE, n.adapt = 1000)
Sys.time() - start
saveRDS(out, file = "MS_Scenarios/ACP.4.5.rds")
################################################################################
# Current conditions
...
jags.data.current <- list(obs.ice = ice.data$ice_current, nb.size = theta.current, 
                          ...
                          nest.obs.b = c(fecund.param$ns.obs.beta, ns.b),
                          K = K, BEFORE = 4, AFTER = 1)
saveRDS(jags.data.current, file = "MS_Scenarios/jags.data.current.rds")
start <- Sys.time()
out <- jags(jags.data.current, inits, parameters, "ACP_IPM.jags", 
            n.chains = nc, n.burnin=nb, n.iter = ni,  
            parallel = TRUE, n.adapt = 1000)
Sys.time() - start
saveRDS(out, file = "MS_Scenarios/ACP.current.rds")
