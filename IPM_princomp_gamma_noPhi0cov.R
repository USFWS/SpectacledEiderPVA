...
betaPhi[1] ~ dnorm(0, 100)
betaPhi[2] ~ dnorm(0, 100)
...
for (t in 1:(n.occasions - 1 + BEFORE + AFTER + K)){
    ...
    logit.phiA[t] <- logit(mean.phiA*(1-lead[t]*(1-kappa))) 
                     + betaPhi[1]*pca.ice[t+1]
                     + betaPhi[2]*pca.ice[t+1]*pca.ice[t+1]
                     - betaN[2]*(N[3,t] + N[4,t])/1000 + eps.phi[2,t] 
    logit.phi2[t] <- logit(mean.phiA) + betaPhi[1]*pca.ice[t+1] 
                     + betaPhi[2]*pca.ice[t+1]*pca.ice[t+1] 
                     - betaN[2]*(N[3,t] + N[4,t])/1000 + eps.phi[2,t]
