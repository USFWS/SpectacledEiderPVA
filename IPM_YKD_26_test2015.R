#based on IMP_YKD_26.R with only change to add back in year 2015 count data to 
#  see if the year-specific d and handling to huge observation effect for that year. 
#  testing only on YKD_IPM_L2008.jags
...
### Lead 2008
cat(file = "YKD_IPM_L2008_test2015.jags", "
model {
...
eiders[[2]]
counts <- eiders[[1]]  |> #read.csv("input_data/YKD_SPEI.csv", header = T)
...
# MCMC settings
ni <- 20000; nt <- 1; nb <- 10000; nc <- 4

# Call JAGS from R (jagsUI)
# lead 2008, ice 8.5
start <- Sys.time()
out <- jags(jags.data8.5, inits, parameters, "YKD_IPM_L2008_test2015.jags", 
                      n.chains = nc, n.burnin=nb, n.iter = ni,  
                      parallel = TRUE, n.adapt = 1000)
Sys.time() - start
saveRDS(out, "YKD_IPM_L2008_test2015.rds")