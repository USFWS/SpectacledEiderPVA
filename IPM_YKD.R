### ADAPTED FROM IPM_working_EE1 TO RUN THE 6 SCENARIOS WITH LEAD AND ICE FOR THE
### YKD POPULATION

library(jagsUI)

# reading in the most recent runs
YKD.L2008.4.5 <- readRDS("YKD.L2008.4.5.rds")
YKD.L2026.4.5 <- readRDS("YKD.L2026.4.5.rds")
YKD.constant.4.5 <- readRDS("YKD.constant.4.5.rds")
YKD.L2008.8.5 <- readRDS("YKD.L2008.8.5.rds")
YKD.L2026.8.5 <- readRDS("YKD.L2026.8.5.rds")
YKD.constant.8.5 <- readRDS("YKD.constant.8.5.rds")
YKD.L2019.current <- readRDS("YKD.L2019.current.rds")
...
###### RUNNING THROUGH 6 FUTURE CONDITION SCENARIOS (ALL COMBINATIONS OF TWO ICE
# PROJECTIONS (4.5 AND 8.5) AND 3 LEAD PROJECTIONS (CONSTANT, DECLINE IN 2026, AND
# DECLINE IN 2008)). 
...
# bundle data
## nb.size PARAMETER CHANGES WITH ICE SCENARIO. CURRENT = 4.3, RCP4.5 = 3.75, 
## RCP8.5 = 3.5
jags.data <- list(obs.ice = ice.data8.5, nb.size = 3.5,
...
# Call JAGS from R (jagsUI), use autojags to run to convergence
YKD.L2026.8.5 <- jags(jags.data, inits, parameters, "YKD_IPM.jags", 
                      n.chains = nc, n.burnin=nb, n.iter = ni,  
                      parallel = TRUE, n.adapt = 1000)

saveRDS(YKD.L2026.8.5, file = "YKD.L2026.8.5.rds")
...
# Call JAGS from R (jagsUI), use autojags to run to convergence
YKD.L2026.4.5 <- jags(jags.data, inits, parameters, "YKD_IPM.jags", 
                      n.chains = nc, n.burnin=nb, n.iter = ni,  
                      parallel = TRUE, n.adapt = 1000)

saveRDS(YKD.L2026.4.5, file = "YKD.L2026.4.5.rds")

### MODELING CURRENT CONDITIONS. DECLINE BEGINNING IN 2008 BUT
# THEN CONSTANT FROM 2019 ON
...
# decline in 2008, remain constant after 2019
for (i in 1:20){
  lead[i] <- theta_0
} # i
for (i in 21:32){
...
for (i in 33:(BEFORE+n.occasions-1+AFTER+K)){
  lead[i] <- lead[32]
}
...
## nb.size PARAMETER CHANGES WITH ICE SCENARIO. CURRENT = 4.3, RCP4.5 = 3.75, 
## RCP8.5 = 3.5
jags.data <- list(obs.ice = ice.datacurrent, nb.size = 4.3,
                  marr = ms.arr, n.occasions = ncol(ch), rel = rowSums(ms.arr), 
                  ns = ns, zero = matrix(0, ncol = ns, nrow = ns), ones = diag(ns), 
                  count = counts$Nibb, obs = observer, sigma.obs = counts$seNibb,
                  nest.obs.a = fecund.param$ns.obs.alpha[1:23],
...
# Call JAGS from R (jagsUI), use autojags to run to convergence
YKD.L2019.current <- jags(jags.data, inits, parameters, "YKD_IPM.jags", 
                              n.chains = nc, n.burnin=nb, n.iter = ni,  
                              parallel = TRUE, n.adapt = 1000)

saveRDS(YKD.L2019.current, file = "YKD.L2019.current.rds")
