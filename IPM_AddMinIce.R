...
for (t in 1:(n.occasions - 1 + BEFORE + AFTER + K)){
    ...
    logit.phiA[t] <- logit(mean.phiA*(1-lead[t]*(1-kappa))) + beta[1]*min.ice[t+1] 
                     + beta[2]*ext.ice[t+1] + beta[3]*ext.ice[t+1]*ext.ice[t+1] 
                     - betaN[2]*(N[3,t] + N[4,t])/1000 + eps.phiA[t] 
    logit.phi2[t] <- logit(mean.phiA) + beta[1]*min.ice[t+1] 
                     + beta[2]*ext.ice[t+1] + beta[3]*ext.ice[t+1]*ext.ice[t+1] 
                     - betaN[2]*(N[3,t] + N[4,t])/1000 + eps.phiA[t]
...
YKD.constant.4.5.addMinIce <- jags(jags.data4.5, inits, parameters, "YKD_IPM.jags", 
