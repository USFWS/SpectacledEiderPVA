### ADAPTED FROM IPM_working_EE1 TO RUN THE 3 SCENARIOS ICE FOR THE
### ACP POPULATION
...
counts <- read.csv("input_data/ACP_SPEI.csv", header = T)
...
mean.logit.BP ~ dnorm(0.472, 0.22) T(-0.77, 3.22) #from EE
betaF.ice ~ dnorm(2.83, 0.11) T(-1, 3) #from EE
betaF.ice2 ~ dnorm(-1.17, 0.69) T(-1.21, 0.33) #from EE
tau.BP <- pow(sigma.BP, -2)
sigma.BP ~ dunif(0,1)
for (i in 1:2){
betaN[i] ~ dgamma(3, 3300) # from EE, DD for phi and F
} #i
...
mean.phi0 ~ dbeta(2.3, 4) # from EE
mean.logit.phi0 <- logit(mean.phi0)
tau.phi0 <- pow(sigma.phi0, -2)
sigma.phi0 ~ dunif(0, 1)

mean.phiA ~ dbeta(26,3) # from EE, approximately, upper 90% quantile is a little high 0.96, not 0.94
...
for (t in 1:(BEFORE+n.occasions-1+AFTER+K)){ # extended loop here
    ...
    N[2,t+1] <- phi1[t]*(1-alpha[t])*N[1,t]
    N[3,t+1] <- phi1[t]*alpha[t]*N[1,t]
    N[4,t+1] <- N[5,t+1] + N[6,t+1]
    N[5,t+1] <- phi2[t]*N[2,t]
    N[6,t+1] <- phiA[t]*(N[3,t] + N[4,t])
} # t
...
sigma.o ~ dgamma(1, 10)
tau.d <- pow(sigma.d, -2)
sigma.d ~ dgamma(1, 13) # from EE
for(i in 1:8){
  o[i] ~ dnorm(0, tau.o)
} # i
for (t in 1:(n.occasions+BEFORE+AFTER)){
  d[t] ~ dnorm(0, tau.d) T(-0.28,0.28) #bounded so that detection is not > 1 or < 0.5 (assuming mean detection ~0.75)
...
for (t in 1:(n.occasions+BEFORE+AFTER+K-1)){ # extended loop here
  logit.BP[t] <- mean.logit.BP + betaF.ice*ice[t] + betaF.ice2*ice[t]*ice[t] + eps.BP[t]
  eps.BP[t] ~ dnorm(0, tau.BP)
  BP[t] <- ilogit(logit.BP[t])
  F[t] <- exp(log(phi0[t]) + mean.log.F 
...
for (t in 1:(n.occasions - 1 + BEFORE + AFTER + K)){
    ...
    logit.phiA[t] <- mean.logit.phiA + beta[1]*ice[t+1] + beta[2]*ice[t+1]*ice[t+1] 
...
# bundle data
jags.data <- list(obs.ice = ice.data4.5, nb.size = 3.75,
...
parameters <- c("Nb", "mean.logit.phi0", "mean.logit.phiA", "betaN", "beta", "mean.log.F", 
               "sigma.phiA", "sigma.phi0", "sigma.d", "sigma.o", "o",
               "betaF.ice", "betaF.ice2", "mean.logit.BP")

# MCMC settings
nb <- 20000; nc <- 4

# Call JAGS from R (jagsUI), use autojags to run to convergence
ACP.4.5 <- jags(jags.data, inits, parameters, "ACP_IPM.jags", 
                 n.chains = nc, n.burnin=nb, n.iter=30000,
                 parallel = TRUE, n.adapt = 1000)

saveRDS(ACP.4.5, file = "ACP_IPM_Scenarios/ACP.4.5.rds")

summary(ACP.4.5)
print(ACP.4.5)
hist(ACP.4.5$summary[,"Rhat"])
ACP.4.5$summary[,"Rhat"][ACP.4.5$summary[,"Rhat"]>1.1]

##----------------------------------------------------------------------
## RPC8.5 ice scenario
jags.data$obs.ice <- ice.data8.5
jags.data$nb.size <- 3.5
ACP.8.5 <- jags(jags.data, inits, parameters, "ACP_IPM.jags", 
                n.chains = nc, n.burnin=nb, n.iter=30000,
                parallel = TRUE, n.adapt = 1000)

saveRDS(ACP.8.5, file = "ACP_IPM_Scenarios/ACP.8.5.rds")
summary(ACP.8.5)
print(ACP.8.5)
hist(ACP.8.5$summary[,"Rhat"])
ACP.8.5$summary[,"Rhat"][ACP.8.5$summary[,"Rhat"]>1.1]

##----------------------------------------------------------------------
## current ice scenario
jags.data$obs.ice <- ice.datacurrent
jags.data$nb.size <- 4.3
ACP.current <- jags(jags.data, inits, parameters, "ACP_IPM.jags", 
                n.chains = nc, n.burnin=nb, n.iter=30000,
                parallel = TRUE, n.adapt = 1000)

saveRDS(ACP.current, file = "ACP_IPM_Scenarios/ACP.current.rds")
summary(ACP.current)
print(ACP.current)
hist(ACP.current$summary[,"Rhat"])
ACP.current$summary[,"Rhat"][ACP.current$summary[,"Rhat"]>1.1]
