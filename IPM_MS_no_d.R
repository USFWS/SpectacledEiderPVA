...
sigma.BP ~ dunif(0,1)
for (i in 1:2){
betaN[i] ~ dgamma(3.11, 251.6) # from EE, see DDpriorsEO.R
} #i
...
for (t in 1:(n.occasions+BEFORE+AFTER)){
  count[t] ~ dnorm(2*exp(log(N[3,t] + N[4,t]) - o[obs[t]]), tau.obs[t]) # count provided as data
...
ggplot(L2008.4.5fig, aes(x = Year, y = Nb/1000, col = Sim)) +
  ...
  geom_point(data = counts, aes(x = Year, y = Nibb/1000), size = 2, shape = 1, col = "black") +
